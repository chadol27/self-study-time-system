...
- 구현 전에 [DETAILS.md](DETAILS.md)의 관련 구현 정책을 읽고 출결 상태, 시트 구조, 인증, 동시성, 날짜 경계 규칙을 따른다.
- 설치·배포·운영 절차는 [README.md](README.md)를 따른다. 문서와 실행 코드가 다르면 `src/appsscript.json`, `.claspignore`, 실제 코드를 우선 확인한다.
- 코드를 수정하면 변경된 동작과 정책을 `DETAILS.md`에, 설치·배포·운영 영향을 `README.md`에 함께 반영한다.
...
- 날짜·출결 변경은 `DETAILS.md`의 `17. 핵심 테스트 항목`을 기준으로 검증한다.
