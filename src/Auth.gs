...
function sessionPropertyKey_(hash) {
  return "TEACHER_SESSION_" + hash;
}
function cleanupSessions_() {
  ...
  Object.keys(all)
    .filter(function (k) {
      return k.indexOf("TEACHER_SESSION_") === 0 && all[k] !== today;
    })
    .forEach(function (k) {
...
function requireTeacher_(token) {
  if (!token || typeof token !== "string" || token.length < 32)
    throw userError_("교사 인증이 필요합니다.", "AUTH_REQUIRED");
  cleanupSessions_();
  const expiry = PropertiesService.getScriptProperties().getProperty(
    sessionPropertyKey_(hashToken_(token)),
  );
  if (expiry !== todayKey_())
    throw userError_(
      "교사 인증이 만료되었습니다. 다시 로그인해 주세요.",
      "AUTH_EXPIRED",
    );
}
function teacherLogin(password) {
  return publicCall_(function () {
    ...
    cleanupSessions_();
    if (typeof password !== "string" || password !== config.teacherPassword)
      throw userError_("비밀번호가 올바르지 않습니다.", "LOGIN_FAILED");
  ...
  return publicCall_(function () {
    if (token)
      PropertiesService.getScriptProperties().deleteProperty(
        sessionPropertyKey_(hashToken_(String(token))),
      );
    return true;
  });
}
