...
function getConfig_() {
  ...
  if (raw.TOTAL_SEATS && (!Number.isInteger(seats) || seats < 1 || seats > 500))
    errors.push("TOTAL_SEATS");
  const times = [
    raw.PERIOD_1_START_TIME,
    raw.PERIOD_2_START_TIME,
    raw.PERIOD_3_START_TIME,
  ];
  times.forEach(function (v, i) {
    if (v && !/^([01]\d|2[0-3]):[0-5]\d$/.test(v))
      errors.push("PERIOD_" + (i + 1) + "_START_TIME");
  });
...
function requireConfig_() {
  ...
  if (!config.ok)
    throw userError_("필수 운영 설정이 올바르지 않습니다.", "CONFIG_INVALID", {
      keys: config.errors,
    });
  return config;
}
...
function publicCall_(callback) {
  try {
    ...
    return {
      ...
      error: {
        code:
          error && error.name === "UserError" ? error.code : "INTERNAL_ERROR",
        message:
          error && error.name === "UserError"
            ? error.message
