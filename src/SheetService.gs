  ...
  try {
    ...
    if (!roster) {
      ...
      if (first && first.getLastRow() === 0 && first.getLastColumn() === 0) {
        first.setName(APP.SHEETS.ROSTER);
        roster = first;
...
function headersValid_() {
  ...
  return (
    ...
    APP.ROSTER_HEADERS_2.every(function (v, i) {
      return values[1][i] === v;
    })
  );
}
...
function getAttendanceColumns_() {
  ...
  const values = sheet
    .getRange(1, APP.ATTENDANCE_FIRST_COL, 2, width)
    .getValues();
  for (let i = 0; i < width; i += 3) {
    const raw = values[0][i];
    const key =
      raw instanceof Date && !isNaN(raw.getTime()) ? dateKey_(raw) : "";
    result.push({
      key: key,
      col: APP.ATTENDANCE_FIRST_COL + i,
      raw: raw,
      periods: values[1].slice(i, i + 3),
    });
  }
  return result;
}
...
function ensureDateColumns_(key) {
  ...
  if (found.length > 1)
    throw userError_("날짜 출결 열이 중복되어 있습니다.", "INVALID_HEADERS");
  if (
  ...
  const later = existing.find(function (x) {
    return x.key > key;
  });
  if (later) {
    col = later.col;
    sheet.insertColumnsBefore(col, 3);
  } else if (sheet.getMaxColumns() < col + 2)
    sheet.insertColumnsAfter(
      sheet.getMaxColumns(),
      col + 2 - sheet.getMaxColumns(),
    );
  const date = parseDateKey_(key);
  sheet
    .getRange(1, col, 1, 3)
    .setValues([[date, date, date]])
    .setNumberFormat("M/d")
    .setFontWeight("bold")
    .setHorizontalAlignment("center");
  sheet
...
function getClosedDates_() {
  ...
  return new Set(
    sheet
      ...
      .getValues()
      .map(function (r) {
        return r[0] instanceof Date && !isNaN(r[0].getTime())
          ? dateKey_(r[0])
          : "";
      })
      .filter(Boolean),
...
function validateAll_() {
  ...
  if (
    ...
    (logSheet.getMaxColumns() < APP.LOG_HEADERS.length ||
      logSheet
        ...
        .getDisplayValues()[0]
        .some(function (v, i) {
          return v !== APP.LOG_HEADERS[i];
        }))
  )
    errors.push("기록 시트 헤더가 올바르지 않습니다.");
  if (
    settingsSheet &&
    (settingsSheet.getMaxColumns() < APP.SETTINGS_HEADERS.length ||
      settingsSheet
        .getRange(1, 1, 1, APP.SETTINGS_HEADERS.length)
        .getDisplayValues()[0]
        .some(function (v, i) {
          return v !== APP.SETTINGS_HEADERS[i];
        }))
  )
  ...
  students.forEach(function (s) {
    if (s.key) {
      if (keys[s.key]) {
        errors.push("내부 학생 키 중복: " + s.studentId);
        excluded.add(s.key);
        excluded.add(keys[s.key]);
      } else keys[s.key] = s.key;
    } else {
      errors.push("내부 학생 키 누락: " + s.studentId);
      excluded.add(s.key);
    }
    if (s.active) {
    ...
    if (!Number.isInteger(s.seat) || s.seat < 1 || s.seat > config.totalSeats) {
      errors.push(
        "좌석번호 범위 오류: " + s.studentId + " (" + s.seatRaw + ")",
      );
      excluded.add(s.key);
    }
    s.applications.forEach(function (v, i) {
  ...
  Object.keys(activeIds).forEach(function (id) {
    if (id && activeIds[id].length > 1) {
      errors.push("활성 학번 중복: " + id);
      activeIds[id].forEach(function (s) {
        excluded.add(s.key);
      });
    }
  });
